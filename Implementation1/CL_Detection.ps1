...
$Global:FirewallRuleHomeGroup    = "@provsvc.dll,-202"                                                              # IDS_FIREWALL_GROUP
...
{
    [string]$path = "HKLM:\SYSTEM\CurrentControlSet\Services\HomeGroupProvider\ServiceData"
...
function Check-WindowsFirewallRules()
{
    ...
    if (-not($issueDetected) -and (Test-HomeGroupName))
    {
        ...
        # File sharing firewall rules are only enabled on non-domain-joined machines
        if (-not($issueDetected) -and -not(IsDomainJoined))
        {
