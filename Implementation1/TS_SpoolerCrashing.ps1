...
$date = Date
$date = $date.AddDays(-1)
$event = get-winevent -FilterHashTable @{ ProviderName = 'Application Error'; StartTime = $date; Data="spoolsv.exe"; Id = 1000 }
...
if($event -ne $null -and $event.Count -gt 1)
{
    ...
    if($addRootCause)
    {
        ...
        $event | convertto-xml | Update-DiagReport -id SpoolerEvent -name $localizationString.spoolerEvent_name -description $localizationString.spoolerEvent_description -verbosity Error -rid "RC_SpoolerCrashing"
        return
    }
}
